...
int main(int argc, char **argv){
    ...
    while(!client->wait_for_service(1s)){
        ...
        RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "service not available, wait again...");
    }

    auto result = client->async_send_request(request); //这是一个std::future对象
    //thread waiting for result
